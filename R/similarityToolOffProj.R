similarityToolOffProj <- function(battedBallVals, playerVals, numResults=10, year=2017){
  require(FNN)
  numResults = numResults + 1
  names = battedBallVals[,"Name"] 
  if(numResults > nrow(battedBallVals)) {numResults = nrow(battedBallVals) - 1}
  convertedVals<- battedBallVals[,c('LD%','GB%','FB%','Pull%','Cent%','Oppo%','Soft%','Med%','Hard%', 'HR/FB', 'K%', 'BB%','Spd')]
