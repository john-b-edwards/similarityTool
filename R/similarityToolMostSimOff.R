similarityToolMostSimOff <- function(battedBallVals, numResults){
  ...
  battedBallVals$NameTwo = paste(battedBallVals$Name, as.character(battedBallVals$Season), sep='')
  names = battedBallVals[,"NameTwo"]
  ...
  names(result)[names(result)=="NameTwo"] <- "Most Similar Player"
