similarityToolMostUniquePitching <- function(battedBallVals, numResults){
  ...
  for (row in 1:nrow(names)) {
    name = names[row,]
    result = rbind(result, similarityToolPitching(battedBallVals, substr(toString(name),0,nchar(toString(name))-4), 1, as.double(substr(toString(name),nchar(toString(name))-3,nchar(toString(name))))))
  }
